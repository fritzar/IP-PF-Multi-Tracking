...
for i=1:100 %点目标扩散
    for x=1:50
        for y=1:50
            h(x,y,i)=exp(-((x-x_dis(1,i))^2/(2*dx))*lx-((y-x_dis(3,i))^2/(2*dy))*ly);
        end
    end
end
...
u=zeros(50,50,Total_time); %扩散目标测量
...
for t_i=1:100 %计算点目标扩散的参数u
    for x=1:50
        for y=1:50
            u(x,y,t_i)=Frame_data(x,y,t_i)*h(x,y,t_i)+2*wgn(1,1,0)^2;
        end
    end
end
figure(20)
...
%                 keyboard;
            ...
            E_target_state_MC(:,:,:,1,1,1)=E_target_state;
end   
