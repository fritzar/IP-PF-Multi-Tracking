...
%             display(['NP=',num2str(Np),'; SNR=',num2str(SNR_dB),'; MC=',num2str(MC_i)]);           
 ...
 for tt=1:Total_time
     for xx=1:50
         for yy=1:50
             jl=round(sqrt((xx-x_dis(1,tt))^2+(yy-x_dis(3,tt))^2));
             if (jl<1)
               Frame_data(xx,yy,tt)=Frame_data(x_dis(1,tt),x_dis(3,tt),tt)*1;
             else if (jl<2)
               Frame_data(xx,yy,tt)=Frame_data(x_dis(1,tt),x_dis(3,tt),tt)*0.5+Frame_data(xx,yy,tt);
             else if (jl<3)
               Frame_data(xx,yy,tt)=Frame_data(x_dis(1,tt),x_dis(3,tt),tt)*0.2+Frame_data(xx,yy,tt);
             else
               Frame_data(xx,yy,tt)=Frame_data(xx,yy,tt);
             end
            end
        end
         end
...
figure(t);
 ...
 %figure(1);
 %plot(E_target_state_MC(1,:,1),E_target_state_MC(4,:,1), 'b.-', 'markersize',5);
 %hold on;
 %plot(E_target_state_MC(1,:,2),E_target_state_MC(4,:,2), 'r.-', 'markersize',5);
 %i=1:100;
 %plot(xn(1,i),xn(3,i),'y.-', 'markersize',5);
 %j=101:200;
 %plot(xn(1,j),xn(3,j),'g.-', 'markersize',5);
 %legend('目标1','目标2','跟踪1','跟踪2');
 %xlabel('x方向/分辨单元');ylabel('y方向/分辨单元');
 %axis([0 50 0 50]);
